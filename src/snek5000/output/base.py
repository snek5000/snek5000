...
class Output(OutputCore):
    ...
    def _set_info_solver_classes(cls, classes):
        ...
        classes._set_child(
            "PhysFields",
            dict(
                module_name="snek5000.output.phys_fields", class_name="PhysFields"
            ),
        )
...
"""
    ...
    def _init_path_session(self):
        ...
        if not self.params or not hasattr(self, "path_run"):
            logger.debug(
                "Attribute sim.output.path_session will not be initialized."
            )
            return None
    ...
    def _get_subpackages(self):
        ...
        subpackages = {
            subpkg.name.replace(f"{root.name}.", ""): self._get_resources(
                subpkg.name
            )
            for subpkg in pkgutil.walk_packages(
                [str(root)], prefix=f"{self.package}."
            )
    ...
    def get_paths(self):
        ...
        paths += [
            self.path_solver_package / resource
    ...
    def write_makefile_usr(self, template, fp=None, **template_vars):
        ...
        for path_dir, list_of_sources in self.makefile_usr_sources.items():
            for sources in list_of_sources:
                paths_of_sources.append(
                    [f"{path_dir}/{file}" for file in sources]
                )

        if mpi.rank == 0:
            comments = (
                "Autogenerated using snek5000.output.Output.write_makefile_usr\n"
            )
    ...
    def write_compile_sh(template, config, fp=None, path=None):
        ...
        if fp:
            ...
            raise ValueError(
                "Either file pointer or the path to it must be provided."
            )

        if path:
            os.chmod(
                path, stat.S_IRWXU | stat.S_IRGRP | stat.S_IWGRP | stat.S_IROTH
            )
    ...
    def get_field_file(self, prefix="", index=-1, t_approx=None):
        ...
        if index > 0 and t_approx:
            raise ValueError(
                "Specify either index or t_approx at a time, not both."
            )
        elif index > 0:
